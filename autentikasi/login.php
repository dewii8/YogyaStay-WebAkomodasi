...
if (!isset($_SESSION['user_id']) && isset($_COOKIE['remember_user'])) {
    ...
    if ($user = mysqli_fetch_assoc($result)) {
        ...
        // CEK STATUS USER SAAT REMEMBER ME
        if ($token === $expected_token && $user['status'] === 'aktif') {
            // Set Session
            ...
            // Hapus cookie jika user dinonaktifkan
        ...
        if ($user = mysqli_fetch_assoc($result)) {
            ...
            if (password_verify($password, $user['password'])) {
                ...
                if ($user['status'] !== 'aktif') {
                    $error = 'Akun Anda telah dinonaktifkan oleh admin. Silakan hubungi administrator untuk informasi lebih lanjut.';
                } else {
                    // SET SESSION
                    $_SESSION['user_id'] = $user['id_user'];
                    $_SESSION['id_user'] = $user['id_user']; 
                    ...
                    // SUCCESS
                    $success = 'Login berhasil! Selamat datang, ' . htmlspecialchars($user['nama']) . ' 👋';
...
<head>
    ...
    <style>
        * {
        ...
        .swal-inactive .swal2-icon.swal2-warning {
...
<body>
    ...
    Swal.fire({
        title: "Oops!",
        text: "<?php echo $error; ?>",
        icon: "<?php echo (strpos($error, 'dinonaktifkan') !== false) ? 'warning' : 'error'; ?>",
        confirmButtonText: "Mengerti",
        confirmButtonColor: "<?php echo (strpos($error, 'dinonaktifkan') !== false) ? '#DC2626' : '#A4A8C1'; ?>",
        willOpen: () => {
            <?php if (strpos($error, 'dinonaktifkan') !== false): ?>
            Swal.getPopup().classList.add('swal-inactive');
            <?php else: ?>
