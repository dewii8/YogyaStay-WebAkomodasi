...
.amenity-icon {
    font-size: 18px;
    color: #70777eff !important; /* Warna yang sama dengan beranda */
}
...
<div class="card-amenities">
    <?php
    $prioritas_fasilitas = ['WiFi', 'AC', 'TV']; // Urutan prioritas
    ...
    // Tidak ada fallback - jika tidak ada fasilitas, area ini akan kosong
    ?>
</div>
...
document.getElementById('kabupaten').addEventListener('change', function () {
    ...
    if (kabupatenId) {
        ...
        fetch('?ajax=kecamatan&id_kabupaten=' + kabupatenId)
            .then(res => res.text())
            .then(data => {
                // Tambahkan opsi "Semua Kecamatan" di awal
                kecamatanSelect.innerHTML = '<option value="">Semua Kecamatan</option>' + data;
            });
    } else {
        // Jika pilih "Semua Kabupaten", tampilkan "Semua Kecamatan"
        kecamatanSelect.innerHTML = '<option value="" selected>Semua Kecamatan</option>';
    }
});

// Hapus handler focus dan blur yang tidak diperlukan
// Karena sekarang "Semua Kecamatan" selalu menjadi opsi default yang valid
