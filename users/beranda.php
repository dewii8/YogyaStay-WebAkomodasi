...
const checkinPicker = flatpickr("#checkin", {
    ...
    onChange: function(selectedDates, dateStr, instance) {
        if (selectedDates[0]) {
            instance.input.value = formatDateCustom(selectedDates[0]);
            ...
            instance.input.setAttribute('data-date', `${y}/${m}/${d}`);
        }
        ...
        if (selectedDates[0]) {
            ...
            if (!document.getElementById('checkout').value) {
                checkoutPicker.setDate(nextDay, false); 
                document.getElementById('checkout').value = formatDateCustom(nextDay);
                const yOut = nextDay.getFullYear();
                const mOut = String(nextDay.getMonth() + 1).padStart(2, '0');
                const dOut = String(nextDay.getDate()).padStart(2, '0');
                document.getElementById('checkout').setAttribute('data-date', `${yOut}/${mOut}/${dOut}`);
            }
        }
    },
    onOpen: function(selectedDates, dateStr, instance) {
        if (selectedDates[0]) {
            instance._originalDate = selectedDates[0];
        }
    },
...
document.getElementById('searchForm').addEventListener('submit', function(e) {
    ...
    // Ambil tanggal dari data attribute atau parse dari display value
    const checkinDate = checkinInput.getAttribute('data-date') || checkinPicker.selectedDates[0];
    const checkoutDate = checkoutInput.getAttribute('data-date') || checkoutPicker.selectedDates[0];
    ...
    if (document.querySelector('.penginapan-container')) {
        ...
        filterButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                // "LIHAT SEMUA" BERDASARKAN TAB YANG AKTIF
                if (kabupatenId === 'all') {
...
// MAPPING ICON FASILITAS - BOOTSTRAP ICONS
const facilityIcons = {
    'WiFi': 'bi-wifi',
    'AC': 'bi-snow',
    'TV': 'bi-tv',
...
function createPenginapanCard(penginapan) {
    ...
    const rating = 4.8;
    const reviewCount = 172;
    const starsHTML = '<i class="bi bi-star-fill"></i>'; 

    // RENDER FASILITAS - LANGSUNG ARRAY
    let fasilitasHTML = '';
    if (penginapan.fasilitas && Array.isArray(penginapan.fasilitas) && penginapan.fasilitas.length > 0) {
        fasilitasHTML = penginapan.fasilitas.map(fas => {
            const iconClass = facilityIcons[fas] || 'bi-check-circle';
            return `<div class="facility"><i class="bi ${iconClass}"></i></div>`;
        }).join('');
    } else {
        // Fallback jika tidak ada fasilitas
        fasilitasHTML = `
            <div class="facility"><i class="bi bi-cup-hot"></i></div>
        `;
    }
    ...
    return `
        <div class="penginapan-card">
            <div style="position: relative;">
                ...
                <div class="penginapan-type">${penginapan.tipe_penginapan || 'Hotel'}</div>
            </div>
