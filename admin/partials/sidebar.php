...
/* ========== DYNAMIC BASE URL ========== */
// Mendapatkan protocol (http atau https)
$protocol = isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http";

// Mendapatkan host (localhost atau domain)
$host = $_SERVER['HTTP_HOST'];

// Mendapatkan path dari document root
...
if ($adminIndex !== false) {
    // Ambil path sampai sebelum 'admin'
    $baseParts = array_slice($pathParts, 0, $adminIndex);
    $baseUrl = $protocol . "://" . $host . implode('/', $baseParts);
} else {
    // Fallback: ambil 2 level ke atas dari current directory
...
// Logout handler - HARUS SETELAH $baseUrl didefinisikan
    ...
    if (isTablet) {
        menuLinks.forEach(link => {
            link.addEventListener('click', function() {
                // Close sidebar after a short delay to show the click animation
                setTimeout(() => {
                    toggleSidebar();
                }, 200);
            });
