...
/*  TIPE KAMAR  */
// JANGAN DELETE SEMUA, tapi UPDATE yang ada
...
// 2. Hapus tipe kamar lama dari database (yang tidak ada booking aktif)
if (!empty($existing_tipe)) {
    $ids = implode(',', $existing_tipe);
    
    // CEK mana yang aman dihapus (tidak ada booking aktif)
...
if (!empty($_POST['tipe_kamar'])) {
    foreach ($_POST['tipe_kamar'] as $i => $namaTipe) {
        ...
        // CEK apakah nama tipe sudah ada (untuk UPDATE)
        ...
        /*  GAMBAR PENGINAPAN (bukan tipe kamar)  */
            ...
            foreach ($_FILES['gambar_penginapan']['tmp_name'] as $i => $tmp) {
                if ($_FILES['gambar_penginapan']['error'][$i] === 0) {
                    ...
                    if (move_uploaded_file($tmp, $folder.$file)) {
                        // FIX: Jika ini gambar pertama yang diupload dan belum ada thumbnail, set sebagai thumbnail
        ...
        // LOG AKTIVITAS ADMIN - DIPERBAIKI DENGAN AUTO-DETECT KOLOM
        if (isset($_SESSION['user_id'])) {
            ...
            if ($check_table) {
                ...
                if ($admin_column && in_array('aksi', $columns)) {
                    ...
                    // Debug jika gagal (opsional - bisa dihapus nanti)
                    if (!$log_result) {
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="form-grid">
            <div class="form-group form-full">
                ...
                <input type="text" name="nama_penginapan" value="<?= htmlspecialchars($data['nama_penginapan']) ?>" required>
            </div>
            ...
            <div class="form-group">
                ...
                <select name="tipe_penginapan" required>
                    ...
                    <option value="hotel" <?= strtolower($data['tipe_penginapan']) === 'hotel' ? 'selected' : '' ?>>Hotel</option>
                    <option value="villa" <?= strtolower($data['tipe_penginapan']) === 'villa' ? 'selected' : '' ?>>Villa</option>
                </select>
            </div>
        ...
        <div class="form-grid">
            <div class="form-group">
                ...
                <input type="tel" name="kontak" value="<?= htmlspecialchars($kontak_data['telepon'] ?? '') ?>" placeholder="0274-123456">
            </div>

            <div class="form-group">
                <label>Nomor WhatsApp</label>
                <input type="tel" name="no_wa" value="<?= htmlspecialchars($kontak_data['whatsapp'] ?? '') ?>" placeholder="08123456789">
            </div>

            <div class="form-group form-full">
                <label>Email</label>
                <input type="email" name="email" value="<?= htmlspecialchars($kontak_data['email'] ?? '') ?>" placeholder="info@penginapan.com">
            </div>
        </div>
        ...
        <div class="gallery-preview" id="gallery-preview">
            ...
            ?>
                <div class="img-preview-item" data-gambar-id="<?= $img['id_gambar'] ?>">
                    <img src="../../<?= htmlspecialchars($img['path_gambar']) ?>" alt="Preview" onerror="this.src='../../uploads/penginapan/default.jpg'">
                    <?php if ($img['is_thumbnail']): ?>
                        <div class="img-thumbnail-badge">Thumbnail</div>
                    <?php endif; ?>
                    <button type="button" class="btn-delete-img" onclick="hapusGambar(<?= $img['id_gambar'] ?>, this)" title="Hapus gambar">
...
<script>
// Load kecamatan saat halaman dimuat
