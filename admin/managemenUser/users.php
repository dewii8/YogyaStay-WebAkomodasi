...
if (isset($_GET['delete_id'])) {
    ...
    if ($user_data) {
        ...
        try {
            // Hapus data terkait user (gunakan ON DELETE CASCADE jika sudah di-set di foreign key)
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-buttons">
                                <form method="POST" action="users.php?page=<?= $page; ?>" style="margin:0;" id="form-<?= $row['id_user']; ?>">
                                    <input type="hidden" name="user_id" value="<?= $row['id_user']; ?>">
                                    <select name="status" class="status-dropdown" data-user-id="<?= $row['id_user']; ?>" data-user-name="<?= htmlspecialchars($row['nama']); ?>" data-current-status="<?= $row['status']; ?>">
...
document.querySelectorAll('.status-dropdown').forEach(select => {
    select.addEventListener('change', function(e) {
        ...
        // Jika status tidak berubah, tidak perlu konfirmasi
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                // Jika dibatalkan, kembalikan ke status semula
